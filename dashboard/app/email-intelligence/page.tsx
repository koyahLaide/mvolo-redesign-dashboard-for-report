        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">Email Intelligence — timing, segmenten & aanbevelingen</p>
            </div>
            <Nav />
          </div>
          {report && (
            <span className="text-xs text-gray-600">
              Rapport: {new Date(report.generated_at).toLocaleDateString('nl-NL', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
            </span>
          )}
        </div>
      </header>
          ...
          <>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-gray-900 border border-indigo-900/40 rounded-xl px-5 py-4">
                    <p className="text-xs text-gray-500">Beste dag om te sturen</p>
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-800">
                    <h2 className="text-sm font-semibold text-gray-300">Omzet per email per dag van de week</h2>
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-800">
                    <h2 className="text-sm font-semibold text-gray-300">Orders per dag van de week (alle kanalen)</h2>
                    <p className="text-xs text-gray-600 mt-0.5">Wanneer zijn klanten het meest actief?</p>
                  </div>
                  <div className="p-6 space-y-3">
                    {(data?.orderDow ?? []).map((d: any) => (
                      <div key={d.dow} className="flex items-center gap-4">
                        <span className="text-sm text-gray-300 w-24 flex-shrink-0">{DAYS[parseInt(d.dow)]}</span>
            ...
            {tab === 'campaigns' && (
              <div className="space-y-6">
                {report?.campaign_performance && (
                  <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-800">
                      <h2 className="text-sm font-semibold text-gray-300">Campaign performance (omzet 3 dagen na verzending)</h2>
                ...
                {data?.campaignOrders?.length > 0 && (
                  <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-800">
                      <h2 className="text-sm font-semibold text-gray-300">UTM-geattribueerde orders per campaign</h2>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-4">
                    <p className="text-xs text-gray-500">Nieuwe klanten</p>
                    ...
                    <div className="mt-3 space-y-1">
                      <p className="text-xs text-gray-400 font-medium">Beste kanalen:</p>
                      {newCustomers.sort((a: any, b: any) => b.orders - a.orders).slice(0, 4).map((r: any) => (
                        <div key={r.channel} className="flex justify-between text-xs">
                          <span className="text-gray-500">{r.channel?.replace(/_/g,' ')}</span>
                          <span className="text-gray-300">{r.orders} orders, €{r.aov}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-4">
                    <p className="text-xs text-gray-500">Terugkerende klanten</p>
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-xl px-6 py-5 space-y-4">
                  <h2 className="text-sm font-semibold text-gray-300">Email strategie per segment</h2>
                      ...
                      {[
                        ...
                        <div key={f.flow} className="bg-gray-800/40 rounded-lg px-4 py-3">
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-medium text-gray-300">{f.flow}</span>
            ...
            {tab === 'producten' && (
              <div className="space-y-6">
                <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-800">
                    <h2 className="text-sm font-semibold text-gray-300">Top producten via email kanaal</h2>
                      ...
                      {data.emailSkus.map((s: any, i: number) => (
                        <div key={s.sku} className="px-6 py-3 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm text-gray-300 truncate max-w-sm">{s.title?.substring(0, 50)}</p>
                          ...
                          <div className="flex items-center gap-6 text-xs text-right flex-shrink-0">
                            <span className="text-gray-500">{s.sold} verkocht</span>
