        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">Finance — winst & verlies · BTW · returns</p>
            </div>
            <Nav />
          </div>
          <div className="flex items-center gap-2">
            {/* Maand/Kwartaal switch */}
            <div className="flex items-center bg-gray-900 border border-gray-800 rounded-lg p-0.5">
              <button onClick={() => setMode('monthly')}
                className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${mode === 'monthly' ? 'bg-white text-gray-900' : 'text-gray-400'}`}>
                Per maand
              </button>
              <button onClick={() => setMode('quarterly')}
                className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${mode === 'quarterly' ? 'bg-white text-gray-900' : 'text-gray-400'}`}>
                Per kwartaal
              </button>
            </div>
            {data?.usdEurRate && (
        ...
        {!loading && (
          <div className="grid grid-cols-4 gap-4">
            <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-4">
              ...
              <p className="text-2xl font-bold text-white mt-1">{formatEuro(totalRevExcl)}</p>
              <p className="text-xs text-gray-600 mt-1">Laatste periode: {formatEuro(latest.revenue_excl)}</p>
            </div>
            <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-4">
              <p className="text-xs text-gray-500">Brutomarge (12m)</p>
              <p className="text-2xl font-bold text-green-400 mt-1">{formatEuro(totalGross)}</p>
              <p className="text-xs text-gray-600 mt-1">{totalRevExcl > 0 ? Math.round((totalGross / totalRevExcl) * 100) : 0}% marge</p>
            </div>
            <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-4">
              ...
              <p className="text-xs text-gray-600 mt-1">na OPEX + ad spend</p>
            </div>
            <div className="bg-yellow-950/30 border border-yellow-900/30 rounded-xl px-5 py-4">
              <p className="text-xs text-gray-500">BTW saldo (12m)</p>
              <p className="text-2xl font-bold text-yellow-400 mt-1">{formatEuro(totalBtwTeBetalen)}</p>
              <p className="text-xs text-gray-600 mt-1">ontvangen − betaald</p>
            </div>
          </div>
        )}
          ...
          <>
              ...
              <div className="space-y-4">
                <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-800">
                    <h2 className="text-sm font-semibold text-gray-300">Winst & Verlies {mode === 'quarterly' ? 'per kwartaal' : 'per maand'}</h2>
                    <p className="text-xs text-gray-600 mt-0.5">Omzet excl. BTW − COG − OPEX − Ad spend = Nettoresultaat</p>
                  </div>
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b border-gray-800 text-gray-500 uppercase tracking-wider">
                          ...
                          <th className="px-4 py-2 text-right font-bold text-white">Netto</th>
                          <th className="px-4 py-2 text-right">Netto%</th>
                        </tr>
                      </thead>
                      <tbody>
                        {pnl.slice(-12).map((r: any) => (
                          <tr key={r.month ?? r.quarter} className="border-b border-gray-800/50 hover:bg-gray-800/20">
                            ...
                            <td className="px-4 py-2.5 text-right text-white font-semibold tabular-nums">{formatEuro(r.revenue_excl)}</td>
                            <td className="px-4 py-2.5 text-right text-gray-400 tabular-nums">{r.cog > 0 ? formatEuro(r.cog) : '—'}</td>
                            <td className="px-4 py-2.5 text-right text-green-400 tabular-nums">{formatEuro(r.gross_profit)}</td>
                            <td className="px-4 py-2.5 text-right tabular-nums">
                              <span className={r.gross_margin_pct > 50 ? 'text-green-400' : r.gross_margin_pct > 30 ? 'text-yellow-400' : 'text-red-400'}>
                                {r.gross_margin_pct}%
                              </span>
                            </td>
                      ...
                      <tfoot>
                        <tr className="border-t border-gray-700 bg-gray-800/30">
                          <td className="px-4 py-2.5 text-gray-300 font-semibold text-xs">Totaal</td>
                          ...
                          <td className="px-4 py-2.5 text-right text-white font-bold tabular-nums text-xs">
                            {formatEuro(totalRevExcl)}
                          </td>
                          <td className="px-4 py-2.5 text-right text-gray-400 tabular-nums text-xs">
                            {formatEuro(pnl.reduce((s: number, r: any) => s + r.cog, 0))}
                          </td>
                          <td className="px-4 py-2.5 text-right text-green-400 font-bold tabular-nums text-xs">
            ...
            {tab === 'btw' && (
              <div className="space-y-6">
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-800">
                    <h2 className="text-sm font-semibold text-gray-300">BTW overzicht {mode === 'quarterly' ? 'per kwartaal' : 'per maand'}</h2>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b border-gray-800 text-gray-500 uppercase tracking-wider">
                          <th className="px-4 py-2 text-left">Periode</th>
                          <th className="px-4 py-2 text-right">Omzet incl.</th>
                          <th className="px-4 py-2 text-right text-green-400">BTW ontvangen</th>
                          <th className="px-4 py-2 text-right text-red-400">BTW betaald (OPEX)</th>
                          <th className="px-4 py-2 text-right font-bold text-yellow-400">Saldo te betalen</th>
                        </tr>
                      </thead>
                      <tbody>
                        {pnl.slice(-12).map((r: any) => {
                          ...
                          return (
                            <tr key={key} className="border-b border-gray-800/50 hover:bg-gray-800/20">
                              ...
                              <td className="px-4 py-2.5 text-right text-gray-400 tabular-nums">{formatEuro(r.revenue_incl)}</td>
                              <td className="px-4 py-2.5 text-right text-green-400 tabular-nums">{formatEuro(r.btw_ontvangen)}</td>
                              <td className="px-4 py-2.5 text-right text-red-400 tabular-nums">{formatEuro(r.btw_betaald)}</td>
                              <td className="px-4 py-2.5 text-right font-bold tabular-nums">
                                <span style={{ color: r.btw_te_betalen > 0 ? '#eab308' : '#22c55e' }}>
                      ...
                      <tfoot>
                        <tr className="border-t border-gray-700 bg-gray-800/30">
                          ...
                          <td className="px-4 py-2.5 text-right text-red-400 font-bold tabular-nums text-xs">
                            {formatEuro(pnl.reduce((s: number, r: any) => s + r.btw_betaald, 0))}
                          </td>
                          <td className="px-4 py-2.5 text-right font-bold tabular-nums text-xs text-yellow-400">
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-4">
                    ...
                    <p className="text-2xl font-bold text-white mt-1">{formatEuro(data?.opex ?? 0)}</p>
                  </div>
                  <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-4">
                    <p className="text-xs text-gray-500">Per week</p>
                    <p className="text-2xl font-bold text-gray-300 mt-1">{formatEuro((data?.opex ?? 0) / 4.33)}</p>
                  </div>
                  <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-4">
                    <p className="text-xs text-gray-500">Per dag</p>
                    <p className="text-2xl font-bold text-gray-300 mt-1">{formatEuro((data?.opex ?? 0) / 30)}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                    <div className="px-5 py-4 border-b border-gray-800">
                      <h2 className="text-sm font-semibold text-gray-300">Per categorie</h2>
                    </div>
                        ...
                        return (
                          <div key={cat.category} className="px-5 py-3 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <span className="text-xs text-gray-300 capitalize">{cat.category}</span>
                              <span className="text-xs text-gray-600">{cat.items}x</span>
                            </div>
                            <div className="flex items-center gap-3 text-xs text-right">
                              ...
                              <span className="text-white font-semibold w-16">{formatEuro(cat.monthly)}</span>
                      ...
                      {(data?.opexDetail ?? []).filter((item: any) => item.category !== 'salary').map((item: any, i: number) => (
                        <div key={i} className="px-5 py-2 flex items-center justify-between">
                          <div>
                            ...
                            <span className="text-xs text-gray-600 ml-2 capitalize">{item.frequency === 'yearly' ? 'jaarlijks' : 'maandelijks'}</span>
                          </div>
                          <div className="text-xs text-right">
                            <span className="text-white">{formatEuro(item.monthly_amount)}</span>
                            {item.btw > 0 && <span className="text-gray-600 ml-2">+€{item.btw} BTW</span>}
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {tab === 'returns' && (
              <div className="space-y-6">
                <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-800">
                    <h2 className="text-sm font-semibold text-gray-300">Return rate per kanaal</h2>
                    <p className="text-xs text-gray-600 mt-0.5">Gebaseerd op financial_status = refunded of partially_refunded</p>
                  </div>
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {(data?.returnByChannel ?? []).map((r: any) => {
                        ...
                        return (
                          <tr key={r.channel} className="border-b border-gray-800/50 hover:bg-gray-800/20">
                            ...
                            <td className="px-5 py-2.5 text-right text-gray-300 tabular-nums text-xs">{r.returns}</td>
                            <td className="px-5 py-2.5 text-right tabular-nums text-xs">
                              <span style={{ color: r.pct > 10 ? '#ef4444' : r.pct > 5 ? '#f59e0b' : '#22c55e' }}>
                                {r.pct}%
                              </span>
                            </td>
                            <td className="px-5 py-2.5 text-right text-red-400 tabular-nums text-xs">
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-800">
                    <h2 className="text-sm font-semibold text-gray-300">Returns {mode === 'quarterly' ? 'per kwartaal' : 'per maand'}</h2>
                  </div>
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-gray-800 text-gray-500 uppercase tracking-wider">
                    ...
                    <tbody>
                      {pnl.filter((r: any) => r.returns > 0).map((r: any) => (
                        <tr key={r.month ?? r.quarter} className="border-b border-gray-800/50 hover:bg-gray-800/20">
                          ...
                          <td className="px-4 py-2 text-right text-gray-300 tabular-nums">{r.returns}</td>
                          <td className="px-4 py-2 text-right tabular-nums">
                            <span style={{ color: r.return_rate > 10 ? '#ef4444' : r.return_rate > 5 ? '#f59e0b' : '#22c55e' }}>
                              {r.return_rate}%
                            </span>
                          </td>
                          <td className="px-4 py-2 text-right text-red-400 tabular-nums">{formatEuro(r.return_value)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
