    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-8 py-8 space-y-6">
      ...
      {data && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl px-5 py-4">
            <p className="text-xs text-gray-500 dark:text-gray-400">Winback kandidaten</p>
            <p className="text-2xl font-bold text-red-500 dark:text-red-400 mt-1">{totalWinbackKlanten}</p>
          ...
          <div className="bg-white dark:bg-gray-900 border border-red-200 dark:border-red-900/30 rounded-xl px-5 py-4">
            <p className="text-xs text-gray-500 dark:text-gray-400">Verloren LTV</p>
            <p className="text-2xl font-bold text-red-500 dark:text-red-400 mt-1">{formatEuro(totalWinbackLtv)}</p>
            <p className="text-xs text-gray-400 dark:text-gray-600 mt-1">historische waarde winback klanten</p>
          </div>
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl px-5 py-4">
            <p className="text-xs text-gray-500 dark:text-gray-400">Beste LTV kanaal</p>
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl px-5 py-4">
            <p className="text-xs text-gray-500 dark:text-gray-400">Snelste herhaalaankoop</p>
            <p className="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">25 dagen</p>
      ...
      {loading ? (
        ...
        <>
                    ...
                    return (
                      <div key={ch.first_channel} className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-4 text-xs text-right flex-shrink-0 w-72">
                          <span className="text-gray-900 dark:text-white font-semibold w-16">€{ch.avg_ltv}</span>
                          <span className="text-gray-500 dark:text-gray-400 w-16">{ch.klanten} klanten</span>
                          <span className="text-gray-600 dark:text-gray-400 w-20">{ch.avg_orders_per_klant}x orders</span>
                          <span className="text-gray-600 dark:text-gray-400 w-20">max €{ch.max_ltv}</span>
                        </div>
                      </div>
                    );
                  })}
                    ...
                    {(data?.returnRate ?? []).map((r: any) => {
                      ...
                      return (
                        <tr key={r.first_channel} className="border-b border-gray-200/50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/20">
                          ...
                          <td className="px-5 py-2.5 text-right text-gray-500 dark:text-gray-400 tabular-nums">{r.eerste_orders}</td>
                          <td className="px-5 py-2.5 text-right text-gray-700 dark:text-gray-300 tabular-nums font-semibold">{r.herhaalaankopen}</td>
                          <td className="px-5 py-2.5 text-right tabular-nums">
                            <span className={`font-semibold ${r.herhaal_rate_pct > 10 ? 'text-green-500 dark:text-green-400' : r.herhaal_rate_pct > 5 ? 'text-yellow-500 dark:text-yellow-400' : 'text-gray-500 dark:text-gray-400'}`}>
                              {r.herhaal_rate_pct}%
                            </span>
                          </td>
                    ...
                    {data.newByMonth.map((r: any) => {
                      ...
                      return (
                        <div key={r.month} className="flex items-center gap-3">
                          <span className="text-xs text-gray-500 dark:text-gray-400 w-16 flex-shrink-0">{r.month}</span>
                          <div className="flex-1 h-4 bg-gray-200 dark:bg-gray-800 rounded overflow-hidden flex">
                            <div className="h-full bg-green-500/60 dark:bg-green-600/60" style={{ width: `${newPct}%` }} />
                            <div className="h-full bg-indigo-500/60 dark:bg-indigo-600/60" style={{ width: `${100 - newPct}%` }} />
                          </div>
                          <div className="text-xs text-right w-40 flex-shrink-0">
                            <span className="text-green-600 dark:text-green-400">{r.nieuw} nieuw</span>
                            <span className="text-gray-400 dark:text-gray-600 mx-1">·</span>
                            <span className="text-indigo-600 dark:text-indigo-400">{r.terugkerend} terugk.</span>
                          </div>
                        </div>
                      );
                    })}
                    <div className="flex items-center gap-4 mt-2 text-xs text-gray-400 dark:text-gray-600">
                      <span><span className="text-green-600 dark:text-green-400">■</span> Nieuw</span>
                      <span><span className="text-indigo-600 dark:text-indigo-400">■</span> Terugkerend</span>
                    </div>
                  </div>
                </div>
              )}
          ...
          {tab === 'winback' && (
            <div className="space-y-6">
              <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/30 rounded-xl px-6 py-5">
                <h2 className="text-sm font-semibold text-red-600 dark:text-red-400 mb-3">Winback strategie</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs text-gray-600 dark:text-gray-400">
                  <div className="bg-white dark:bg-gray-900/60 rounded-lg px-4 py-3 border border-gray-200 dark:border-gray-800">
                    <p className="font-semibold text-gray-900 dark:text-white">60-90 dagen inactief</p>
                    <p className="mt-1">Stuur een "We missen je" email met een kleine korting (10%). Klant is nog actief in geheugen.</p>
                  </div>
                  <div className="bg-white dark:bg-gray-900/60 rounded-lg px-4 py-3 border border-gray-200 dark:border-gray-800">
                    <p className="font-semibold text-gray-900 dark:text-white">90-180 dagen inactief</p>
                    <p className="mt-1">Stuur scarcity offer of nieuwe productlancering. Grotere korting (15-20%) om terug te winnen.</p>
                  </div>
                  <div className="bg-white dark:bg-gray-900/60 rounded-lg px-4 py-3 border border-gray-200 dark:border-gray-800">
                    <p className="font-semibold text-gray-900 dark:text-white">180+ dagen inactief</p>
              ...
              <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-bold text-red-500 dark:text-red-400">{totalWinbackKlanten} klanten</p>
                    ...
                    return (
                      <div key={r.laatste_kanaal} className="flex items-center gap-4 bg-gray-50 dark:bg-gray-800/30 rounded-xl px-4 py-3">
                        ...
                        <div className="flex items-center gap-4 text-xs text-right flex-shrink-0 w-80">
                          <span className="text-red-500 dark:text-red-400 font-semibold w-20">{r.klanten} klanten</span>
                          <span className="text-gray-500 dark:text-gray-400 w-24">gem. {r.gem_dagen_geleden}d geleden</span>
                          <span className="text-gray-500 dark:text-gray-400 w-16">€{r.avg_ltv} LTV</span>
                          <span className="text-indigo-600 dark:text-indigo-400 font-semibold w-20">{formatEuro(r.totale_ltv)}</span>
                        </div>
                      </div>
                    );
                  })}
          ...
          {tab === 'window' && (
            <div className="space-y-6">
              <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
                ...
                <div className="p-6 space-y-3">
                  {(data?.purchaseWindow ?? []).map((r: any, i: number) => (
                    <div key={r.window} className="flex items-center gap-4">
                      <span className="text-xs text-gray-500 dark:text-gray-400 w-28 flex-shrink-0">{r.window}</span>
                ...
                <div className="px-6 pb-5 text-xs text-gray-500 dark:text-gray-400 space-y-1">
                  <p>→ <span className="text-green-600 dark:text-green-400">23 klanten</span> kopen binnen 30 dagen opnieuw — stuur cross-sell email na 14 dagen</p>
                  <p>→ <span className="text-indigo-600 dark:text-indigo-400">5 klanten</span> kopen na 90-180 dagen — stuur winback na 75 dagen</p>
                  <p>→ Zet je <span className="text-gray-900 dark:text-white font-medium">Winback flow</span> op 60 dagen als eerste trigger</p>
                </div>
              </div>
            </div>
          )}
          ...
          {tab === 'site' && (
            <div className="space-y-6">
              <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-gray-200 dark:border-gray-800 text-gray-500 dark:text-gray-400">
                        ...
                        <th className="px-5 py-2 text-right text-red-500 dark:text-red-400">Niet gekocht</th>
                      ...
                      {(data?.siteJourney ?? []).filter((r: any) => r.product_views > 0).reverse().slice(0, 20).map((r: any) => {
                        ...
                        return (
                          <tr key={r.date} className="border-b border-gray-200/50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/20">
                            <td className="px-5 py-2 text-gray-500 dark:text-gray-400">{new Date(r.date).toLocaleDateString('nl-NL', { day: 'numeric', month: 'short' })}</td>
                            <td className="px-5 py-2 text-right text-gray-700 dark:text-gray-300 tabular-nums">{r.product_views}</td>
                            <td className="px-5 py-2 text-right text-blue-500 dark:text-blue-400 tabular-nums">{r.cart_adds || '—'}</td>
                            <td className="px-5 py-2 text-right text-yellow-500 dark:text-yellow-400 tabular-nums">{r.checkouts || '—'}</td>
                            <td className="px-5 py-2 text-right text-green-500 dark:text-green-400 tabular-nums">{r.orders || '—'}</td>
                            <td className="px-5 py-2 text-right text-red-500 dark:text-red-400 font-semibold tabular-nums">{notBought}</td>
                            <td className="px-5 py-2 text-right tabular-nums">
                              <span className={convRate > 20 ? 'text-green-500 dark:text-green-400' : convRate > 10 ? 'text-yellow-500 dark:text-yellow-400' : 'text-red-500 dark:text-red-400'}>
                                {convRate}%
                              </span>
                            </td>
                          </tr>
                ...
                <div className="px-6 py-4 bg-indigo-50 dark:bg-indigo-950/20 border-t border-gray-200 dark:border-gray-800/50 text-xs text-gray-500 dark:text-gray-400">
                  <p>→ Bekende Klaviyo contacten die een product bekijken maar niet kopen triggeren de <span className="text-indigo-600 dark:text-indigo-400">Browse Abandonment flow</span></p>
                  <p className="mt-1">→ Je Browse Abandonment flow is <span className="text-green-600 dark:text-green-400">live</span> — zorg dat de trigger timing op max 1 uur staat</p>
                </div>
              </div>
            </div>
          )}
