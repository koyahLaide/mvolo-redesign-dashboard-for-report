...
export async function GET() {
  try {
    const vel30Result = await pool.query(`
      SELECT sku, SUM(quantity) * 1.0 / 30 as daily
      FROM order_items
      WHERE order_date >= CURRENT_DATE - INTERVAL '30 days' AND sku != ''
      GROUP BY sku
    `);
    const vel90Result = await pool.query(`
      SELECT sku, SUM(quantity) * 1.0 / 90 as daily
      FROM order_items
      WHERE order_date >= CURRENT_DATE - INTERVAL '90 days' AND sku != ''
      GROUP BY sku
    `);

    const v30: Record<string, number> = {};
    const v90: Record<string, number> = {};
    vel30Result.rows.forEach((r: any) => { v30[r.sku] = parseFloat(r.daily); });
    vel90Result.rows.forEach((r: any) => { v90[r.sku] = parseFloat(r.daily); });
    ...
    const products = cogsData.products
      .map((p: any) => {
        ...
        if (velocity === 0) {
          urgency = stock <= 0 ? 'KRITIEK' : 'GEEN DATA'; method = 'SEA'; reorder_qty = 0; cogs = p.cogs_sea;
        } else if (stock <= 0) {
          urgency = 'KRITIEK';
          ...
          urgency = 'AIR URGENT'; method = 'AIR';
          reorder_qty = Math.max(0, Math.ceil(velocity * 60) - stock); cogs = p.cogs_air;
          ...
          urgency = 'BESTEL SEA'; method = 'SEA';
          reorder_qty = Math.max(0, Math.ceil(velocity * 90) - stock); cogs = p.cogs_sea;
        } else {
          urgency = 'OK'; method = 'SEA'; reorder_qty = 0; cogs = p.cogs_sea;
        }

        const reorder_cost = cogs && reorder_qty > 0 ? Math.round(reorder_qty * cogs) : 0;

        return {
          name: p.name, sku: p.sku, stock, price: inv.price,
          cogs_sea: p.cogs_sea, cogs_air: p.cogs_air, air_allowed: p.air_allowed,
          velocity_30d: Math.round(daily30 * 100) / 100, velocity_90d: Math.round(daily90 * 100) / 100,
          days_left: days_left > 900 ? null : days_left,
          urgency, method, reorder_qty, reorder_cost, sea_lead, air_lead,
        };
      })
      .filter(Boolean);
