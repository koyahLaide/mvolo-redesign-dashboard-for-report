...
export default function PrijzenPage() {
  ...
  const maxRevenue = Math.max(...(data?.channelWithMargin ?? []).map((c: any) => c.revenue), 1);
        ...
        <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-1 w-fit">
          {([
            ...
            <button key={key} onClick={() => setTab(key)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${tab === key ? 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200'}`}>
              {label}
            </button>
          ))}
        </div>
              ...
              <div className="space-y-6">
                ...
                {alerts.length > 0 && (
                  <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/40 rounded-xl px-5 py-4">
                    <p className="text-xs font-semibold text-red-600 dark:text-red-400 uppercase tracking-wider mb-2">⚠ Marge alerts</p>
                    <div className="space-y-1">
                      {alerts.map((p: any, i: number) => (
                        <div key={`${p.sku}-${i}`} className="flex items-center justify-between text-xs">
                          <span className="text-gray-700 dark:text-gray-300">{p.name}</span>
                          <span className="text-red-600 dark:text-red-400">{p.alert} — meest verkocht op €{p.best_volume_price} met {p.best_volume_margin}% marge</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-800">
                    <h2 className="text-sm font-semibold text-gray-800 dark:text-gray-300">Marge per kanaal</h2>
                    <p className="text-xs text-gray-500 mt-0.5">Gewogen gemiddelde op basis van verkochte SKUs × COGS</p>
                  </div>
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-xs uppercase tracking-wider border-b border-gray-200 dark:border-gray-800 text-gray-500">
                    ...
                    <tbody>
                      {(data?.channelWithMargin ?? []).map((ch: any) => {
                        const color = channelColor(ch.channel);
                        const pct = Math.round((ch.revenue / maxRevenue) * 100);
                        return (
                          <tr key={ch.channel} className="border-b border-gray-100 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/20">
                            ...
                            <td className="px-5 py-3 text-right tabular-nums">
                              {ch.gross_profit !== null ? <span className="text-green-600 dark:text-green-400 font-semibold">{formatEuro(ch.gross_profit)}</span> : <span className="text-gray-400">—</span>}
                            </td>
              ...
              <div className="space-y-6">
                ...
                {withTests.length > 0 && (
                  <div className="bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800/40 rounded-xl px-5 py-4">
                    <p className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider mb-3">💡 Aanbevolen prijstests</p>
                    <div className="grid grid-cols-2 gap-3">
                      {withTests.slice(0, 6).map((p: any, i: number) => (
                        <div key={`${p.sku}-${i}`} className="bg-white dark:bg-gray-900/60 rounded-lg px-4 py-3 border border-gray-200 dark:border-gray-800/40">
                          <p className="text-xs font-semibold text-gray-900 dark:text-white">{p.name}</p>
                          <div className="flex items-center gap-3 mt-1">
                            <span className="text-xs text-gray-500">Nu: <span className="text-gray-900 dark:text-white">€{p.test_advice.current_price}</span> ({p.test_advice.current_margin}%)</span>
                            <span className="text-gray-400">→</span>
                            <span className="text-xs text-indigo-600 dark:text-indigo-400 font-semibold">Test: €{p.test_advice.test_price} ({p.test_advice.projected_margin}%)</span>
                          </div>
                          <p className="text-xs text-gray-500 mt-1">{p.test_advice.reason}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                    <div>
                      <h2 className="text-sm font-semibold text-gray-800 dark:text-gray-300">Prijs × volume per product</h2>
                      <p className="text-xs text-gray-500 mt-0.5">Op welke prijs wordt elk product het meest verkocht?</p>
                    </div>
                  </div>
                  <div className="divide-y divide-gray-200 dark:divide-gray-800/50">
                    {(data?.priceAnalysis ?? []).map((p: any, pi: number) => {
                        ...
                        <div key={`${p.sku}-${pi}`}>
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              {p.stock !== null && (
                                <span className={`text-xs px-1.5 py-0.5 rounded ${p.stock <= 0 ? 'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400' : p.stock <= 5 ? 'bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-500'}`}>
                            ...
                            <div className="flex items-center gap-6 text-xs text-right shrink-0">
                              ...
                              <span className="text-gray-600 dark:text-gray-300">Meest: <span className="text-gray-900 dark:text-white font-semibold">€{p.best_volume_price}</span></span>
                            ...
                            <div className="px-5 pb-4 bg-gray-50 dark:bg-gray-800/10">
                              <div className="space-y-2 pt-2">
                                {p.price_points.map((r: any, i: number) => {
                                  ...
                                  const isBest = i === 0;
                                  return (
                                    <div key={`${r.price}-${i}`} className="flex items-center gap-3">
                                      <span className="text-xs text-gray-600 dark:text-gray-400 w-16 text-right shrink-0 tabular-nums">€{Number(r.price).toFixed(2)}</span>
                                      ...
                                      <span className="text-xs text-gray-500 w-24 truncate">{r.channels}</span>
                                      {isBest && <span className="text-xs text-indigo-600 dark:text-indigo-400">← best</span>}
                                    </div>
                                  );
                                })}
                              </div>
                              {p.test_advice && (
                                <div className="mt-3 bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800/30 rounded-lg px-4 py-2">
                                  <p className="text-xs text-indigo-600 dark:text-indigo-400">💡 Test aanbeveling: {p.test_advice.reason}</p>
                                </div>
                              )}
                            </div>
                          )}
                  ...
                  return (
                    <div key={channel} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
                      <div className="px-5 py-3 border-b border-gray-200 dark:border-gray-800 flex items-center gap-2">
                        ...
                        <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">{formatLabel(channel)}</span>
                      </div>
                      <div className="divide-y divide-gray-100 dark:divide-gray-800/40">
                        {products.map((p: any, i: number) => (
                          <div key={`${p.sku}-${i}`} className="px-5 py-2.5 flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <span className="text-xs text-gray-400">{i + 1}</span>
                              <span className="text-xs text-gray-700 dark:text-gray-300 truncate max-w-45">{p.title?.substring(0, 35)}</span>
                            </div>
                            <div className="flex items-center gap-3 text-xs text-right shrink-0">
                              <span className="text-gray-500">{p.qty}x</span>
                              <span className="text-gray-600 dark:text-gray-400">€{Number(p.avg_price).toFixed(0)}</span>
