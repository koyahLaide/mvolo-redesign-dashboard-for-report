      ...
      <main className="max-w-7xl mx-auto px-8 py-8 space-y-6">
        ...
        {loading ? (
          ...
          <>
                  ...
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {(data?.predictiveReorder ?? []).map((p: any) => {
                        ...
                        return (
                          <tr key={p.sku} className="border-b border-gray-200/50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/20">
                            ...
                            <td className="px-5 py-2.5 text-right text-indigo-400 tabular-nums">{p.cogs_sea ? formatEuro(p.total_cost_seasonal) : '—'}</td>
                      ...
                      {data?.predictiveReorder?.length === 0 && (
                        <tr><td colSpan={9} className="px-5 py-8 text-center text-gray-600">Alle voorraden OK voor komende maand</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
                        ...
                        <div key={i} className="px-5 py-3">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3 text-xs text-right flex-shrink-0 ml-3">
                              <span className="text-indigo-400 font-semibold">{p.samen_gekocht}x</span>
                              <span className="text-gray-600">samen</span>
                            </div>
                          </div>
                        </div>
                      ))}
                        ...
                        <div key={i} className="px-5 py-3">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-xs text-right flex-shrink-0 ml-3 space-y-0.5">
                              <p className="text-green-400 font-semibold">{p.cross_sell_freq}x gekocht</p>
                              <p className="text-gray-600">{p.trigger}</p>
                              {p.stock_b !== null && p.stock_b <= 0 && (
                                <p className="text-red-400">⚠ Uitverkocht</p>
                              )}
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="grid grid-cols-3 gap-4 text-xs">
                    {[
                      ...
                      <div key={i} className="bg-gray-50 dark:bg-gray-50 dark:bg-gray-900/60 rounded-lg px-4 py-3 border border-gray-200 dark:border-gray-800/40">
                        ...
                        <p className="text-gray-700 mt-1">{r.reden}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {tab === 'margin' && (
              <div className="space-y-6">
                <div className="bg-yellow-950/20 border border-yellow-900/30 rounded-xl px-5 py-3 text-xs text-gray-500">
                      ...
                      {(data?.contributionMargin ?? []).map((ch: any) => {
                        ...
                        return (
                          <tr key={ch.channel} className="border-b border-gray-200/50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/20">
                            ...
                            <td className="px-5 py-2.5 text-right tabular-nums">
                              {ch.gross_margin_pct !== null ? <span className="text-green-400">{ch.gross_margin_pct}%</span> : '—'}
                            </td>
              ...
              <div className="space-y-6">
                <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
                    ...
                    {(data?.seasonWeights ?? []).map((s: any) => {
                      ...
                      return (
                        <div key={s.month} className={`flex items-center gap-4 ${isNext ? 'bg-indigo-50 dark:bg-indigo-950/30 rounded-lg px-3 -mx-3' : ''}`}>
                          ...
                          <div className="text-xs text-gray-500 w-32 text-right flex-shrink-0">
                  ...
                  <div className="px-6 py-4 bg-gray-800/30 border-t border-gray-200 dark:border-gray-800/50 space-y-1 text-xs text-gray-500">
                    ...
                    <p>→ Predictive reorder past automatisch de bestelhoeveel­heden aan op basis van deze gewichten</p>
                  </div>
                </div>
              </div>
            )}
